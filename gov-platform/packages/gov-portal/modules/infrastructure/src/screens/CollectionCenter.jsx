...
import { collectionCenterSchema } from "../forms/collection-center.schema";
...
export default function CollectionCenter() {
  ...
  const handleSubmit = async (vals) => {
    try {
      ...
      show("Submitting Collection Center data...");
      const payload = {
        module: "INFRASTRUCTURE_DATA_UPDATE",
        operation: "CREATE",
        formType: "COLLECTION_CENTER_DATA",
      ...
      submitMutation.mutate(
        { method, url: Url, payload },
        {
          onSuccess: (response) => {
            hide();
            enqueue({
              message: "Collection Center data submitted successfully.",
              severity: "success",
              duration: 6000,
            });
            ...
            const params = new URLSearchParams({
              status: "success",
              heading: "Submission Successful For Collection Center Data",
              form: "Collection Center",
              body: "Your Collection Center data has been successfully created.",
  ...
  return (
    <DynamicForm
      ...
      entityId="collection-center"
      autosaveMs={800}
      ui={ui}
      validationSchema={collectionCenterSchema}
      defaultsSchema={collectionCenterSchema}
      output="flat"
