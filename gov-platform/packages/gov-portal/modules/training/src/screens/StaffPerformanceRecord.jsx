...
import { staffPerformanceRecordSchema } from "../forms/staff-performance-record.schema";
...
export default function StaffPerformanceRecord() {
  ...
  const handleSubmit = async (vals) => {
    try {
      ...
      show("Adding Staff Performance Record...");
      const payload = {
        module: "TRAININGS_AND_KM_DATA_UPDATE",
        operation: "CREATE",
        formType: "STAFF_PERFORMANCE",
      ...
      submitMutation.mutate(
        { method, url: Url, payload },
        {
          onSuccess: (response) => {
            hide();
            enqueue({
              message: "Staff Performance Record added successfully.",
              severity: "success",
              duration: 6000,
            });
            ...
            const params = new URLSearchParams({
              status: "success",
              heading: "Submission Successful For Staff Performance Record",
              form: "Staff Performance Record",
              body: "Your Staff Performance Record has been successfully created.",
  ...
  return (
    <DynamicForm
      schema={staffPerformanceRecordSchema}
      formApiRef={formApiRef}
      onSubmit={handleSubmit}
      entityId="staff-performance-record"
      autosaveMs={800}
      ui={ui}
      validationSchema={staffPerformanceRecordSchema}
      defaultsSchema={staffPerformanceRecordSchema}
      output="flat"
