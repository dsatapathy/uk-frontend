...
import { pashuSakhiTrainingFormSchema } from "../forms/pashu-sakhi-training-form.schema";
...
export default function PashuSakhiTraining() {
  ...
  const handleSubmit = async (vals) => {
    try {
      ...
      show("Adding Pashu Sakhi Training details — please wait...");
      const payload = {
        module: "LIVELIHOOD_ACTIVITY_UPDATE",
        operation: "CREATE",
        formType: "PASHU_SAKHI_DATA",
      ...
      submitMutation.mutate(
        { method, url: Url, payload },
        {
          onSuccess: (response) => {
            hide();
            enqueue({
              message: "Pashu Sakhi Training details added successfully.",
              severity: "success",
              duration: 6000,
            });
            ...
            const params = new URLSearchParams({
              status: "success",
              heading: "Submission Successful For Pashu Sakhi Training Addition",
              form: "Pashu Sakhi Training Details",
              body: "Your Pashu Sakhi Training details have been successfully created.",
              reference: response?.data?.data?.pashuId || response?.data?.pashuId || "00XX00",
            }).toString();
  ...
  return (
    <DynamicForm
      schema={pashuSakhiTrainingFormSchema}
      formApiRef={formApiRef}
      onSubmit={handleSubmit}
      entityId="pashu-sakhi-training-form"
      autosaveMs={800}
      ui={ui}
      validationSchema={pashuSakhiTrainingFormSchema}
      defaultsSchema={pashuSakhiTrainingFormSchema}
      output="flat"
