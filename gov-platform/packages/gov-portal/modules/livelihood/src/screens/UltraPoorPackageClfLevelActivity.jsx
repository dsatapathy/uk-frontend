...
import { ultraPoorPackageClfLevelActivitySchema } from "../forms/ultra-poor-package-clf-level-activity.schema";
...
function flatFromMock(data, update = false) {
  return {
    ...
    // --- Status details ---
    statusSection: data?.statusSection ? String(data.statusSection) : "",
...
export default function UltraPoorPackageClfLevelActivity() {
  ...
  const handleValuesChange = React.useCallback(async (vals, meta) => {
    ...
    try {
      show("Loading Ultra Poor Activity Detail for Selected Local ID — please wait...");
      ...
      submitMutation.mutate(
        { method, url: Url, payload },
        {
          onSuccess: (response) => {
            hide();
            enqueue({
              message: isUpdate ? "Ultra Poor CLF Level Activity updated successfully" : "Ultra Poor CLF Level Activity submitted successfully",
              severity: "success",
              duration: 6000,
            });
            ...
            const params = new URLSearchParams({
              status: "success",
              heading: isUpdate ? "Update Successful For Ultra Poor CLF Level Activity" : "Submission Successful For Ultra Poor CLF Level Activity",
              form: "Ultra Poor CLF Level Activity",
  ...
  return (
    <DynamicForm
      schema={ultraPoorPackageClfLevelActivitySchema}
      formApiRef={formApiRef}
      onSubmit={handleSubmit}
      entityId="ultra-poor-package-clf-level-activity"
      autosaveMs={800}
      ui={ui}
      validationSchema={ultraPoorPackageClfLevelActivitySchema}
      defaultsSchema={ultraPoorPackageClfLevelActivitySchema}
      output="flat"
