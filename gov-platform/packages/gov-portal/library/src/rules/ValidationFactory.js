...
function baseForField(field, opts) {
  ...
  switch (t) {
    ...
    default: {
      // For autocomplete, allow string or number
      if (field.type === "autocomplete") {
        let s = z.union([
          z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? undefined : v), z.string()),
          z.preprocess((v) => toNumber(v), z.number())
        ]);
        return allowEmpty ? s.optional() : s;
      }
      // ...existing string logic for other types
      let s = z
        .string()
