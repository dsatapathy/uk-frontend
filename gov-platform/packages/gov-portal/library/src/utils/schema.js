...
export function buildSchema(fields = [], captchaCfg) {
  ...
  for (const f of fields) {
    ...
    if (t === "checkbox") {
      s = z.boolean();
      if (f.required) s = s.refine(Boolean, { message: f.requiredMessage || "This field is required" });
    } else {
      let str = z.string();                           // ← no transform here
      if (f.required)    str = str.min(1, { message: f.requiredMessage || "This field is required" });
